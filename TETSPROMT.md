...

{
  "questions": [{
    ...
    // Type 1 專用：標準答案
    "answer": "正確答案",
    "answerFormat": "matching", // 連連看題型才需要，否則省略
...
【連連看題型（Matching）】
- 判斷為 Type 1
- answer 使用文字格式：左項=右項1,右項2; 左項2=右項3
- 左項目固定，右項目可一對多；同一左項目多個右項目用逗號/頓號分隔
- 題組用分號或換行分隔
- 不要輸出 JSON
- 建議加上 answerFormat: "matching"
...
【引導式多段問答題（全域規則）】
- 識別特徵：步驟一/二、第一步/第二步、先…再…、根據步驟一回答步驟二
- 必須視為「同一題」，不可拆成多題
- idPath 不可新增階層（不可出現 3-2-1 / 3-2-2）
- 題型判斷為 Type 3，使用 rubricsDimensions 分階段

範例：
- 「三、… 2.（步驟一…步驟二…）」→ idPath: ["3","2"] → id: "3-2"
...
function isChineseNumeralChar(char: string): boolean {
  return char in chineseNumberMap || char in chineseUnitMap
}
...
function normalizeIdPathSegment(raw: string): string {
  if (!raw) return ''
  let output = ''
  let buffer = ''

  for (const rawChar of raw) {
    const char = toHalfwidthChar(rawChar)
    if (isChineseNumeralChar(char)) {
      buffer += char
      continue
    }
    if (buffer) {
      const num = chineseNumeralToNumber(buffer)
      if (num !== null) output += String(num)
      buffer = ''
    }
...
function normalizeIdPath(idPath: unknown): string[] | null {
  if (!Array.isArray(idPath)) return null
  const normalized = idPath
    .map((item) => normalizeIdPathSegment(String(item ?? '')).trim())
    .filter(Boolean)
  return normalized.length > 0 ? normalized : null
}

function applyIdPathToAnswerKey(answerKey: AnswerKey): AnswerKey {
  const questions = (answerKey.questions ?? []).map((question) => {
    const normalizedPath = normalizeIdPath(question.idPath)
    if (!normalizedPath) return question
    return {
      ...question,
      idPath: normalizedPath,
...
type MatchingAnswerMap = Map<string, Set<string>>

function normalizeMatchingToken(value: string): string {
  return value.replace(/[\s\u3000]+/g, ' ').trim()
}

function parseMatchingAnswerMap(
  raw: string,
  minPairs: number
): MatchingAnswerMap | null {
  const text = (raw ?? '').replace(/\u3000/g, ' ').trim()
  if (!text || isEmptyStudentAnswer(text)) return null
  ...
  const map: MatchingAnswerMap = new Map()
  const connectorPattern = /(->|→|=>|=|＝|:|：)/

  for (const chunk of chunks) {
    const match = chunk.match(connectorPattern)
    if (!match || match.index === undefined) continue

    const left = normalizeMatchingToken(chunk.slice(0, match.index))
    const rightRaw = normalizeMatchingToken(chunk.slice(match.index + match[0].length))
    ...
    rightItems.forEach((item) => set.add(item))
    map.set(left, set)
  }

  if (map.size < minPairs) return null
  return map
}

function getMatchingAnswerMap(question: AnswerKeyQuestion): MatchingAnswerMap | null {
  if (!question.answer) return null
  const minPairs = question.answerFormat === 'matching' ? 1 : 2
  return parseMatchingAnswerMap(question.answer, minPairs)
}

function areMatchingAnswerMapsEqual(
  expected: MatchingAnswerMap,
  actual: MatchingAnswerMap
): boolean {
  if (expected.size !== actual.size) return false
  for (const [left, expectedRights] of expected) {
    const actualRights = actual.get(left)
    if (!actualRights || actualRights.size !== expectedRights.size) return false
    for (const right of expectedRights) {
      if (!actualRights.has(right)) return false
    }
  }
  return true
}
...
function applyMatchingOverrides(
  ...
  if (!result.details || !Array.isArray(result.details)) return result

  const questionMap = new Map(answerKey.questions.map((q) => [q.id, q]))
  let changed = false

  const details = result.details.map((detail) => {
    const qid = detail.questionId ?? ''
    const question = questionMap.get(qid)
    if (!question || question.type !== 1) return detail

    const expectedMap = getMatchingAnswerMap(question)
    if (!expectedMap) return detail

    const studentMap = parseMatchingAnswerMap(detail.studentAnswer ?? '', 1)
    if (!studentMap) return detail

    const isCorrect = areMatchingAnswerMapsEqual(expectedMap, studentMap)
    const maxScore =
      typeof detail.maxScore === 'number' && Number.isFinite(detail.maxScore)
        ? detail.maxScore
        : question.maxScore ?? 0
    const nextScore = isCorrect ? maxScore : 0

    if (detail.isCorrect === isCorrect && detail.score === nextScore) {
      return detail
    }
    ...
    return {
      ...
      score: nextScore,
      maxScore,
      reason: isCorrect ? '連連看完全一致' : '連連看連線不一致'
    }
  })

  if (!changed) return result
  const totalScore = details.reduce((sum, d) => sum + (d.score ?? 0), 0)
  return { ...result, details, totalScore }
}

const studentAnswerExtractionRules = `
【學生答案擷取規則（機械式抄寫）】
核心原則：原樣輸出，禁止任何形式的修正或推測。
...
- 判斷標準：
  ...
  - 圖上完全沒有任何手繪痕跡 → 輸出「未作答」
  - ?? 即使圖片本身有印刷內容（地圖、座標軸等），只要沒有學生手繪標記，就是未作答！
...
?? 驗證方法：如果你輸出的 studentAnswer 無法在圖片中找到對應的學生筆跡，那就是腦補！
`.trim()

const lowAchievementRules = `
【低成就學生答案處理】
...
const independentJudgementRules = `
【每題獨立判斷原則（防止連鎖腦補）】
?? 嚴重警告：前一題的判斷不可影響後續題目！
...
?? 自我檢查：批改完成後，檢查是否有「連續多題都有內容，沒有任何未作答」的情況。
如果出現這種情況，很可能是發生了連鎖腦補，請重新逐題檢視圖片。
`.trim()

const extractionConfidenceRules = `
【單題擷取信心率（0-100）】
...
3. 連鎖腦補檢查：
   ...
   - ?? 檢查是否有「連續多題都有內容，沒有任何未作答」→ 可能是連鎖腦補
...
export async function gradeSubmission(
  ...
  try {
    ...
    promptSections.push(
      `
你是一沒有感情的機器人，你一定要100%正確地「一字一句」地把學生答案抄寫下來，也不要摘要或縮短，更不要嘗試去理解學生在想什麼，也不要擅自去修正學生的答案。
例如：學生寫給媽媽準備了一個禮物，你就要照字面意思抄寫「給媽媽準備了一個禮物」，不要改成「姊姊們準備了禮物」或「給媽媽準備了一個生日禮物」，也不要改成「給媽媽準備了一份禮物」。
例如：學生寫我不但回來可以看見你們為我準備的東西，還能感受到大大的溫暖，你就要照字面意思抄寫「我不但回來可以看見你們為我準備的東西，還能感受到大大的溫暖」，不要改成「我不但回家可以看見她們為我準備的東西，還感受到大家的深愛。」，也不要改成「我回來可以看見你們為我準備的東西，還能感受到你們的溫暖」。
有了學生的答案後，你需要嚴格比對「學生答案」是否符合「標準答案」不要憑常識亂猜。。
...
`.trim()
    ...
    promptSections.push(studentAnswerExtractionRules)
    promptSections.push(blankAnswerRules)
    promptSections.push(lowAchievementRules)
    promptSections.push(independentJudgementRules)
    promptSections.push(extractionConfidenceRules)
    promptSections.push(finalCheckRules)
...
`.trim()
    ...
    if (answerKey) {
      parsed = applyMatchingOverrides(parsed, answerKey)
    }
...
export async function extractAnswerKeyFromImage(
  ...
  const parsed = JSON.parse(text) as AnswerKey
  return applyIdPathToAnswerKey(parsed)
}
...
`.trim()
  ...
  const result = applyIdPathToAnswerKey(JSON.parse(text) as AnswerKey)
...
`.trim()
  ...
  const result = applyIdPathToAnswerKey(JSON.parse(text) as import('./db').AnswerKey)
