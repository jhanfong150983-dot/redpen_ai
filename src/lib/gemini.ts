...
- **彩色筆跡（粉紅/紅/藍/綠/黃等非黑色）** = 正確答案（學生需填寫部分）
- **黑色印刷文字** = 題目提示（原本就印在上面的）
- **每個方框** = 1 題（不論方框內有幾個字）
...
▸ 如果是「國字注音題」（方框結構：國字｜注音）：
  你現在是一個「紅字提取器」。你的視網膜經過特殊過濾，**完全看不見黑色的印刷文字**。

  👇 請嚴格執行以下三個步驟的「心智處理程序」：
  步驟 1：視覺過濾（Mental Eraser）
  - 掃描整張圖片。
  - 想像你手裡有一瓶「白色塗改液」。
  - **把所有黑色的印刷國字、黑色的注音符號、黑色的題目說明，全部塗成白色（視為背景）。**
  - 現在，圖片中應該只剩下「粉紅色/紅色/洋紅色的筆跡」。

  步驟 2：提取剩餘內容（Extraction）
  - **只讀取**步驟 1 之後剩下的那些紅色/粉紅色字跡。
  - 如果剩下的是國字（例如紅色「耶」），答案就是「耶」。
  - 如果剩下的是注音（例如紅色「ㄆㄚ」），答案就是「ㄆㄚ」。
  - 如果剩下的是打勾符號（例如紅色「✔」），答案就是該選項的編號。
  - ⚠️ 警告：絕對不要修補被你「塗掉」的黑色部分。即使剩下的注音看起來很孤單（如「ㄆㄚ」），也不要試圖去把旁邊黑色的「啪」找回來。

  步驟 3：序列編排
  - 依照「直行閱讀順序」：從圖片**右上角**開始，往**左下角**掃描。
  - 每個獨立的紅色字跡區塊視為 1 題。

  ---
  【防呆檢查表 - 遇到這些情況怎麼辦？】
  Q: 方框裡有一個黑色國字和一個紅色注音，例如 [黑:啪 | 紅:ㄆㄚ]，我該給什麼？
  A: 給 "ㄆㄚ"。因為黑色被你塗掉了，你只看得到紅色。

  Q: 方框裡有一個紅色國字和一個黑色注音，例如 [紅:耶 | 黑:ㄧㄝˊ]，我該給什麼？
  A: 給 "耶"。因為黑色被你塗掉了。

  Q: 題目是「下列敘述何者正確？（複選）」，選項前有紅色打勾，怎麼辦？
  A: 提取所有紅色打勾對應的代號（如 "1、3、4"），視為同一題的答案。

  Q: 我覺得只有注音怪怪的，要不要加上國字？
  A: **絕對不要！** 題目就是要考注音，你補上國字就全錯了。**看到什麼顏色寫什麼字。**
