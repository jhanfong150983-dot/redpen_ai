...
`.trim(),
  ...
  社會: `
【社會作業特別警告：填空題最容易腦補】
⚠️ 嚴重警告：填空題空白時，絕不可依據常識或歷史知識腦補答案！
- 題目：「台灣的首都是______」，學生空白 → 輸出「未作答」（❌ 不可腦補「台北」）
- 題目：「第一次世界大戰發生於______年」，學生空白 → 輸出「未作答」（❌ 不可腦補「1914」）
- 題目：「______是台灣最高峰」，學生空白 → 輸出「未作答」（❌ 不可腦補「玉山」）
- 每題獨立判斷：第1題有寫 ≠ 第2題也該有寫，空白就是空白！
...
2. 位置精準度維度：
   - 檢查位置是否在正確的象限/區域
   - 檢查位置是否符合座標/方位要求（如：121°E 以東、23.5°N 以南）
   - studentAnswer 描述：「位置：第四象限，約 122°E、23°N 附近」

3. 整體判斷：
   - 符號對 + 位置對 → 滿分
   - 符號對 + 位置錯 → 部分分或 0 分（依 criteria 而定）
   - 符號錯 + 位置對 → 部分分或 0 分（依 criteria 而定）
   - 符號錯 + 位置錯 → 0 分

🚨 常見錯誤（絕對禁止）
- ❌ 只看符號對就給滿分，忽略位置是否精準
- ❌ 位置只看「大致在那個象限」，不檢查是否符合座標要求
- ✅ 必須同時檢查符號和位置，兩者都對才能給滿分
...
- 使用 rubricsDimensions 分成兩個維度：
  ...
  2. 位置精準度：{"name": "位置精準度", "maxScore": Y, "criteria": "位置是否符合座標/方位要求（如121°E以東、23.5°N以南）"}
