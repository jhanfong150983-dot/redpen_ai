...
⚠️ 驗證方法：如果你無法在圖片中用手指指出學生寫的每一個字，那就是腦補！
...
2. 位置精準度維度（最重要！）：
   ...
   正確判斷步驟：
   a. 先讀取圖上的經緯度刻度線（如：120°E、121°E、122°E...）
   b. 判斷學生標記的位置在哪兩條經線之間、哪兩條緯線之間
   c. 檢查是否符合題目要求的精準條件（如：「121°E 以東」「23.5°N 以南」）
   d. studentAnswer 必須描述精準位置：「位置：約 122°E、23°N（121°E 以東 ✓、23.5°N 以南 ✓）」
   ...
   ✅ 正確判斷：
   - ✅「位置在 122°E 和 123°E 之間、23°N 和 24°N 之間，符合『121°E 以東、23.5°N 以南』的要求」
   - ✅「位置在 120°E 和 121°E 之間，不符合『121°E 以東』的要求」→ 位置錯誤
...
- 典型情況：
  - 題目印刷是黑色 → 紅筆/藍筆/綠筆手寫內容通常是「範例答案」
  - 題目是彩色印刷 → 與印刷顏色明顯不同的筆跡通常是「範例答案」
- 判斷依據：
  1. 顏色對比：手寫筆跡與印刷文字有明顯色差
  2. 位置線索：寫在答題區、空格處、方框內的不同色筆跡
  3. 筆觸特徵：手寫筆劃 vs 印刷字體
- 應用：優先提取這些不同顏色的手寫內容作為答案
...
設定：Type 3，rubricsDimensions 分維度（圖形正確性、位置精準度、標註完整性）
...
- 使用 rubricsDimensions 分成兩個維度：
  ...
  2. 位置精準度：{"name": "位置精準度", "maxScore": Y, "criteria": "必須讀取圖上經緯度刻度線，精準判斷位置是否符合座標要求（如121°E以東、23.5°N以南），禁止用『大概』『象限』『方向』判斷"}
- ⚠️ 符號對 ≠ 答案對，必須同時檢查符號和位置
- ⚠️ 經緯度題型：位置判斷必須精準，不可用「大致在某象限」「方向正確」等模糊標準
