...
【寫國字 vs 寫注音題型識別 - 顏色判斷最優先！】
🚨 判斷流程（嚴格按照此順序）：
1. 第一優先：尋找「與印刷黑色明顯不同的彩色筆跡」（紅、藍、綠等）
2. 第二步：提取彩色筆跡的內容作為答案（不管是國字還是注音）
3. 第三步：如果沒有明顯彩色筆跡，所有內容顏色相同，才參考題目要求

判斷規則（顏色優先）：
- 有紅色注音、黑色國字 → 提取「紅色注音」（因為紅色 = 老師標記的答案）
  - ✅ 即使題目要求「寫國字」，仍提取紅色注音
- 有藍色國字、黑色注音 → 提取「藍色國字」（因為藍色 = 老師標記的答案）
  - ✅ 即使題目要求「寫注音」，仍提取藍色國字
- 如果國字和注音都是黑色（同色）→ 才依據題目要求判斷

典型場景：
- 題目：「『劈』頭（寫國字）：____」，圖片有黑色「劈」和紅色「ㄆㄧ」
  → 答案抓「紅色ㄆㄧ」（因為紅色 = 答案）
  → ✅ 顏色優先，不管題目說什麼
- 題目：「『ㄆㄧㄣˊ』（寫注音）：____」，圖片有藍色「貧」和黑色「ㄆㄧㄣˊ」
  → 答案抓「藍色貧」（因為藍色 = 答案）
  → ✅ 顏色優先，不管題目說什麼
- 題目：「『劈』頭（寫國字）：____」，圖片只有黑色「劈」（無其他顏色）
  → 答案抓「劈」（因為沒有彩色筆跡，才看題目要求）

⚠️ 記住：彩色筆跡 = 答案，顏色優先級 > 題目要求！圖片中可能同時有國字和注音，優先提取彩色的那個

【「國字題」和「注音題」題型識別 - 每個項目都是獨立的一題！】
🚨 識別特徵：題目包含「國字」或「注音」關鍵字，並列出多個項目
- 題目格式：「寫國字：1.劈__ 2.貧__ 3.困__」
- 題目格式：「寫注音：1.ㄆㄧ__ 2.ㄆㄧㄣˊ__ 3.ㄎㄨㄣˋ__」
- 題目格式：「國字注音：(1)劈__ (2)ㄆㄧㄣˊ__ (3)困__」（混合型）
...
⚠️ 重要：國字和注音可能混在一起考！
- 題目大標題可能寫「國字注音」，但每個小題要根據**實際答案內容**判斷
- 判斷方法：看圖片中該題的彩色筆跡答案是國字還是注音
  - 如果答案是「劈」→ 這題是國字題
  - 如果答案是「ㄆㄧㄣˊ」→ 這題是注音題

範例 1（純國字）：
- 題目：「寫國字：1.劈頭__ 2.貧窮__ 3.困難__」
  → 設定為 3 題，每題答案都是國字：
    - id: "1", answer: "劈"
    - id: "2", answer: "貧"
    - id: "3", answer: "困"
...
範例 2（純注音）：
- 題目：「寫注音：1.ㄆㄧ__ 2.ㄆㄧㄣˊ__ 3.ㄎㄨㄣˋ__」
  → 設定為 3 題，每題答案都是注音：
    - id: "1", answer: "ㄆㄧ"
    - id: "2", answer: "ㄆㄧㄣˊ"
    - id: "3", answer: "ㄎㄨㄣˋ"
...
範例 3（混合型）：
- 題目：「國字注音：(1)劈頭__ (2)ㄆㄧㄣˊ__ (3)困難__」
- 圖片中答案：(1)紅色「劈」 (2)藍色「ㄆㄧㄣˊ」 (3)紅色「困」
  → 設定為 3 題，根據實際答案內容：
    - id: "1", answer: "劈"（國字）
    - id: "2", answer: "ㄆㄧㄣˊ"（注音）
    - id: "3", answer: "困"（國字）

- 題目編號：依圖片上的題號編排（如：「三、國字注音：1.__ 2.__ 3.__」→ 可能是題號 3-1, 3-2, 3-3 或 3.1, 3.2, 3.3）
