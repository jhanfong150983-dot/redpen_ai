...
▸ 如果是「國字注音題」（方框結構：國字｜注音）：
  1. 🎯 絕對顏色鎖定（最高準則）：
   - 掃描圖片中所有方框。
   - 忽略黑色印刷字體（這是題目提示）。
   - **只提取「紅色/粉紅色/洋紅色」的筆跡或字體**（這是標準答案）。
   - 不論該紅色內容是國字還是注音，直接提取。

   2. 🧩 方框內容互斥判定：
   - 每個方框內通常有兩個部分（國字 vs 注音）。
   - 情況 A：看到「紅色國字」+「黑色注音」→ 答案 = 紅色國字。
   - 情況 B：看到「黑色國字」+「紅色注音」→ 答案 = 紅色注音。

   3. ❌ 強制過濾（Negative Constraints）：
   - 嚴禁提取黑色的印刷字體作為答案。
   - 嚴禁將「黑色國字」與「紅色注音」合併輸出（如：不要輸出「啪ㄆㄚ」）。
   - 嚴禁根據「左邊/右邊」的位置來猜測，完全依賴顏色。

   4. 📝 題組勾選題處理（左下角）：
   - 尋找紅色打勾（✔）的位置。
   - 提取該打勾對應的選項數字或內容。
   - 若有多個打勾，合併為單一字串（如 "1、3、4"）。

  【範例推演】
  - 圖片看到方框內：[紅色的 '耶'] [黑色的 'ㄧㄝˊ']
  👉 判定：紅色是 '耶'，答案提取為 "耶"。

  - 圖片看到方框內：[黑色的 '啪'] [紅色的 'ㄆㄚ']
  👉 判定：紅色是 'ㄆㄚ'，答案提取為 "ㄆㄚ"。

  - 圖片看到方框內：[黑色的 '烘'] [紅色的 'ㄏㄨㄥ']
  👉 判定：紅色是 'ㄏㄨㄥ'，答案提取為 "ㄏㄨㄥ"。
